...
package.path = package.path .. string.format(";%s/?.lua;%s/?.xlsx", RAW_PATH, RAW_PATH)
...
-- 全局变量
local CFG = require("excel/xls2lua_config")
...
function CheckCfgCls(cfgClsFileName, cfgClsName, idName)
    local path = string.format("%s/datacls/%s", RAW_PATH, cfgClsFileName)
    local cfgClsFileNameWithoutExt, ext = GetExtension(cfgClsFileName)
    local modPath = string.format("datacls/%s", cfgClsFileNameWithoutExt)
    if FileExists(path) then
        cfgRowCls = require(modPath)
...
function TypeCheck(sheet)
    ...
    for _, typeName in ipairs(sheet.TYPE) do
        local checker = require("xls2lua/typecheck/check" .. typeName)
        table.insert(checks, checker)
    end
    DATA = {}
...
function PostProcessSingleSheet(xls, sheet)
    ...
    local cfgFilePath = string.lower(string.format("%s/%s", LUA_PATH, cfgFileName))
...
function PreProcess()
    ...
    local xlsPath = string.format("%s/excel", RAW_PATH)
    ...
    for _, path in pairs(CFG) do
        ...
        if 0 == ret then
            local modPath = string.format("excel/%s", xlsName)
            require(modPath) -- 导入到 _G 里面
...
function main()
    ...
    if PreProcess() and Process() and PostProcess() then
        local xlsPath = string.format("%s", LUA_PATH)
